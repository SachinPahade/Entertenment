...
export default function TransitionsModal({ children, media_type, id }) {
  ...
  const fetchData = async () => {
    const { data } = await axios.get(
      `https://api.themoviedb.org/3/${media_type}/${id}?api_key=1575a358ab2088aeb1e0079daa7bb074&language=en-US`
    );
  ...
  const fetchVideo = async () => {
    const { data } = await axios.get(
      `https://api.themoviedb.org/3/${media_type}/${id}/videos?api_key=1575a358ab2088aeb1e0079daa7bb074&language=en-US`
    );
